...
import {
  ...
  COCONUT_RENTALS_OG_IMAGE_URL,
} from '@/lib/coconutRentalsAffiliate';
import { escapeHtml, safeDecodeURIComponent } from '@/lib/affiliateRedirectUtils';
...
const VALID_BG_SEGMENT = /^[a-z0-9-]{1,120}$/;
...
export async function GET(request, { params }) {
  ...
  if (!VALID_BG_SEGMENT.test(seg)) {
    return new NextResponse(
      '<!DOCTYPE html><html><head><meta charset="utf-8"><title>Not found</title></head><body><p>Invalid beach gear link.</p></body></html>',
      {
        status: 404,
        headers: {
  ...
  if (seg !== COCONUT_BG_SLUG) {
    return new NextResponse(
      `<!DOCTYPE html><html><head><meta charset="utf-8"><title>Not found</title></head><body><p>Invalid link. Use /bg/${COCONUT_BG_SLUG}</p></body></html>`,
      {
        status: 404,
        headers: {
          'Content-Type': 'text/html; charset=utf-8',
  ...
  const destinationUrl = COCONUT_RENTALS_AFFILIATE_URL;
  const previewTitle = 'Beach Gear Rentals in Aruba';
  const ogDescription =
    'Chairs, umbrellas, coolers, snorkel gear & more—delivered on Aruba. Coconut Rentals via TopTours partner link.';
  const ogImageUrl = COCONUT_RENTALS_OG_IMAGE_URL;
  const pageUrl = `${origin}/bg/${seg}`;

  const loadingHeadline = 'Just a second while we open beach gear rentals in Aruba for you...';
  const loadingSubHtml = isSocialPreviewBot
    ? 'Preparing rich preview details for social platforms.'
    : 'We are opening your beach gear partner link and sending you there in <span class="countdown" id="countdown">1</span> second<span id="plural"> </span>.';
  const loadingStatusInitial = isSocialPreviewBot
    ? 'Preparing social preview.'
    : 'Getting beach gear rentals in Aruba ready...';
  const progressLabel = 'Finalizing your link';
  const statusSteps = [
    'Preparing beach gear rentals in Aruba...',
    'Opening Coconut Rentals...',
