...
import { EXPEDIA_EX_OG_IMAGE_URL } from '@/lib/expediaAffiliate';
import { getExpediaExPreset } from '@/lib/expediaExOptions';
...
const VALID_EX_SEGMENT = /^[a-z0-9-]{1,120}$/;
...
export async function GET(request, { params }) {
  ...
  if (!VALID_EX_SEGMENT.test(seg)) {
    return new NextResponse(
      '<!DOCTYPE html><html><head><meta charset="utf-8"><title>Not found</title></head><body><p>Invalid travel link.</p></body></html>',
      {
        status: 404,
        headers: {
  ...
  const preset = getExpediaExPreset(seg);
  if (!preset) {
    return new NextResponse(
      '<!DOCTYPE html><html><head><meta charset="utf-8"><title>Not found</title></head><body><p>Invalid /ex/ link. Pick a preset from the admin link generator.</p></body></html>',
      {
        status: 404,
        headers: {
          'Content-Type': 'text/html; charset=utf-8',
  ...
  const ogImageUrl = EXPEDIA_EX_OG_IMAGE_URL;
  const pageUrl = `${origin}/ex/${seg}`;

  const partnerName = preset.partner === 'vrbo' ? 'Vrbo' : 'Expedia';
  ...
  const loadingSubHtml = isSocialPreviewBot
    ...
    : `We are opening your ${partnerName} partner link and sending you there in <span class="countdown" id="countdown">1</span> second<span id="plural"> </span>.`;
  const loadingStatusInitial = isSocialPreviewBot
    ? 'Preparing social preview.'
    : `Getting your ${partnerName} link ready...`;
