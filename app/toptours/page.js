import { getLeaderboardTours, getRecentBoosts, updateTourMetadata, getTourPromotionScoresBatch, getTopPromoters } from '@/lib/promotionSystem';
import { getCachedTour, cacheTour } from '@/lib/viatorCache';
import TopToursClient from './TopToursClient';

// Metadata is now handled in layout.js for better SEO structure

export default async function TopToursPage({ searchParams }) {
  const params = await searchParams;
  const scoreType = params?.type || 'all'; // 'all', 'monthly', 'weekly', 'last_month'
  const region = params?.region || null;
  const limit = 20; // Fixed limit of 20 tours
  const offset = 0;
