...
import { isLowValueGuideTag, isHiddenGuide } from '../src/lib/guideIndexing.js';
...
export async function buildSitemapEntries({ baseUrl = defaultBaseUrl(), supabase }) {
  ...
  try {
    ...
    for (const guide of allGuides) {
      ...
      if (isHiddenGuide({ destinationId: guide.destination_id, categorySlug: guide.category_slug })) continue;
  ...
  try {
    ...
    for (const tag of tagRows) {
      ...
      if (isHiddenGuide({ destinationId: tag.destination_id, categorySlug: tag.tag_slug })) continue;
