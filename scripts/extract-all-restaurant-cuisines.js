...
/**
 * Main function to process ALL restaurants across ALL destinations
 */
async function processAllRestaurants(batchSize = 50, maxRestaurants = null) {
  console.log('\n🚀 STARTING CUISINE EXTRACTION FOR ALL RESTAURANTS\n');
  console.log('━'.repeat(60));
  console.log(`📍 Processing ALL restaurants across ALL destinations\n`);
  
  // Build query for all active restaurants
  let query = supabase
    .from('restaurants')
  ...
  while (true) {
    ...
    console.log(`\n📦 Batch ${batchNum}: ${data.length} restaurants`);
    console.log(`   ${restaurantsToProcess.length} need cuisine extraction`);
    console.log(`   ${data.length - restaurantsToProcess.length} already have proper cuisines`);
    ...
    for (const restaurant of restaurantsToProcess) {
      try {
        ...
        if (cuisines.length > 0) {
          ...
          if (success) {
            updated++;
            console.log(`   ✅ ${restaurant.name} (${restaurant.destination_id}): ${cuisines.join(', ')}`);
          } else {
            errors++;
    ...
    if (batchNum % 10 === 0) {
      ...
      console.log(`\n📊 Progress: ${processed} processed, ${updated} updated (${elapsed}s elapsed)`);
    }
  }
  ...
  console.log(`   3. Generate restaurant guides for all destinations`);
}

// Main execution
async function main() {
  const maxRestaurants = process.argv[2] ? parseInt(process.argv[2]) : null; // Optional: limit number for testing
  
  if (maxRestaurants) {
    console.log(`⚠️  Running in TEST MODE: Processing max ${maxRestaurants} restaurants\n`);
  } else {
    console.log(`🌍 Running for ALL restaurants across ALL destinations\n`);
  }
  
  await processAllRestaurants(50, maxRestaurants);
}
