/**
 * Run all Gemini-enhanced SEO scripts for Prague explore pages.
 *
 * Runs in order:
 *   1. Category pages (e.g. /explore/prague/walking-tours-in-prague)
 *   2. Subcategory pages (e.g. /explore/prague/.../best-morning-walking-tours)
 *   3. Tour pages (e.g. /explore/prague/.../prague-castle-walking-tour)
 *
 * Prerequisites:
 *   - Prague data imported: node scripts/import-prague-explore.js "path/to/prague-export.json"
 *   - Optional: node scripts/ingest-prague-tours.js (for Viator descriptions used in tour SEO)
 *   - Run docs/seeds/add_category_page_seo_columns.sql if not already applied
 *
 * Usage (from project root):
 *   node scripts/generate-prague-explore-seo-gemini.js
 *   node scripts/generate-prague-explore-seo-gemini.js --dry-run
 *   node scripts/generate-prague-explore-seo-gemini.js --overwrite
 *   node scripts/generate-prague-explore-seo-gemini.js --limit 2
 *
...
async function main() {
  console.log('🏰 Prague explore SEO: running Gemini scripts for destination=prague\n');
  ...
  console.log('\n✅ Prague explore SEO generation complete.');
}
