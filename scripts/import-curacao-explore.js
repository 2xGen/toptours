/**
 * Import Curaçao explore data from curacao-export.json into v3_landing_* tables.
 * Creates /explore/curacao with same structure as /explore/new-york-city.
 *
 * Usage (from project root):
 *   node scripts/import-curacao-explore.js
 *   node scripts/import-curacao-explore.js "C:\path\to\curacao-export.json"
 *
 * Env: .env.local with NEXT_PUBLIC_SUPABASE_URL, SUPABASE_SERVICE_ROLE_KEY
 * Optional: CURACAO_EXPORT_JSON env or first CLI arg = path to curacao-export.json
 */
...
function getExportPath() {
  const env = process.env.CURACAO_EXPORT_JSON;
  if (env && env.trim()) return path.resolve(env.trim());
  const arg = process.argv[2];
  if (arg && arg.trim()) return path.resolve(arg.trim());
  const defaultPath = path.join(__dirname, '..', 'curacao-export.json');
  if (fs.existsSync(defaultPath)) return defaultPath;
  console.error(
    '❌ No curacao-export.json path. Set CURACAO_EXPORT_JSON or pass path: node scripts/import-curacao-explore.js "C:\\path\\to\\curacao-export.json"'
  );
  process.exit(1);
}
...
async function run() {
  ...
  if (!destination || destination.slug !== 'curacao') {
    console.error('❌ Export must have destination.slug === "curacao"');
    process.exit(1);
  }

  const destSlug = destination.slug;
  const destName = destination.name || 'Curaçao';

  console.log('🗑️  Removing existing Curaçao v3 data...');
