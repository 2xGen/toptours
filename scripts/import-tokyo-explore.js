/**
 * Import Tokyo explore data from tokyo-export.json into v3_landing_* tables.
 * Creates /explore/tokyo with same structure as /explore/new-york-city.
 *
 * Usage (from project root):
 *   node scripts/import-tokyo-explore.js
 *   node scripts/import-tokyo-explore.js "C:\path\to\tokyo-export.json"
 *
 * Env: .env.local with NEXT_PUBLIC_SUPABASE_URL, SUPABASE_SERVICE_ROLE_KEY
 * Optional: TOKYO_EXPORT_JSON env or first CLI arg = path to tokyo-export.json
 */
...
function getExportPath() {
  const env = process.env.TOKYO_EXPORT_JSON;
  if (env && env.trim()) return path.resolve(env.trim());
  const arg = process.argv[2];
  if (arg && arg.trim()) return path.resolve(arg.trim());
  const defaultPath = path.join(__dirname, '..', 'tokyo-export.json');
  if (fs.existsSync(defaultPath)) return defaultPath;
  console.error(
    '❌ No tokyo-export.json path. Set TOKYO_EXPORT_JSON or pass path: node scripts/import-tokyo-explore.js "C:\\path\\to\\tokyo-export.json"'
  );
  process.exit(1);
}
...
async function run() {
  ...
  if (!destination || destination.slug !== 'tokyo') {
    console.error('❌ Export must have destination.slug === "tokyo"');
    process.exit(1);
  }
  ...
  console.log('🗑️  Removing existing Tokyo v3 data...');
