/**
 * List destinations that have BOTH guides AND images (the 213)
 */
...
console.log('\n📚 Fetching destinations with guides AND images...\n');
...
async function main() {
  ...
  const destinationsWithImages = destinationIds
    .map(id => ({
      id,
  ...
  console.log(`✅ Found ${destinationsWithImages.length} destinations with guides AND images\n`);
  console.log('━'.repeat(80));
  console.log('\n📋 DESTINATIONS WITH GUIDES AND IMAGES:\n');
  ...
  sortedRegions.forEach(region => {
    ...
    regionDests.forEach((dest, index) => {
      ...
      console.log(`   ${(totalCount + index + 1).toString().padStart(3)}. 📸 ${sourceIcon} ${dest.name}${dest.country ? ` (${dest.country})` : ''}`);
    });
  ...
  console.log('\n' + '━'.repeat(80));
  console.log(`\n📊 SUMMARY:`);
  console.log(`   Total destinations with guides AND images: ${destinationsWithImages.length}`);
  console.log(`   Hardcoded: ${destinationsWithImages.filter(d => d.source === 'hardcoded').length}`);
  console.log(`   Generated: ${destinationsWithImages.filter(d => d.source === 'generated').length}`);
  console.log('\n');

  // Also create a simple list file
  const simpleList = destinationsWithImages.map((d, i) => 
    `${i + 1}. ${d.name}${d.country ? ` (${d.country})` : ''}`
  ).join('\n');

  const outputPath = path.join(__dirname, '../DESTINATIONS_WITH_GUIDES_AND_IMAGES.txt');
  fs.writeFileSync(outputPath, `DESTINATIONS WITH GUIDES AND IMAGES (${destinationsWithImages.length} total)\n\n${simpleList}`);
