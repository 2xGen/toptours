/**
 * Populate Hardcoded Tours for Middle East Destinations
 * 
 * This script:
 * 1. Filters 8 Middle East destinations from destinationsData.js
 ...
 * - Beirut
 * 
 * Usage: npm run populate-middle-east-tours
 * Or: node scripts/populate-middle-east-tours.js
 */
 ...
 */
async function main() {
  console.log('🚀 Starting import of hardcoded tours for Middle East destinations...\n');

  // Filter only Middle East destinations
  const middleEastDestinations = destinations.filter(dest => 
    MIDDLE_EAST_DESTINATIONS.includes(dest.id)
  );

  // Sort destinations alphabetically
  const sortedDestinations = [...middleEastDestinations].sort((a, b) => 
    a.name.localeCompare(b.name)
  );

  const total = sortedDestinations.length;
  console.log(`📊 Found ${total} Middle East destinations to process:`);
  sortedDestinations.forEach((dest, idx) => {
    console.log(`   ${idx + 1}. ${dest.name} (${dest.id})`);
  });
  console.log('');
  ...
  console.log('='.repeat(60));
  console.log('\n✅ Middle East tours import complete!');
}
