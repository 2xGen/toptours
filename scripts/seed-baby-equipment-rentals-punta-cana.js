/**
 * Seed Punta Cana baby equipment rentals page data
 * Based on BabyQuip content provided by user
 *
 * Usage:
 *   node scripts/seed-baby-equipment-rentals-punta-cana.js
 *
 * Requirements:
...
const puntaCanaData = {
  destination_id: 'punta-cana',
  hero_title: 'Baby Equipment Rentals in Punta Cana',
  hero_description: 'Families and little ones adore Punta Cana. Don\'t want to lug all your baby gear? No problem, we\'re here to help!',
  hero_tagline: 'Families and little ones adore Punta Cana. Don\'t want to lug all your baby gear? No problem, we\'re here to help!',
  
  // Product Categories - Based on available content for Punta Cana (beach destination)
  product_categories: [
    {
      name: 'Cribs & Sleep',
      icon: '🛏️',
    ...
    },
    {
      ...
      icon: '🛴',
      description: 'Lightweight strollers, jogging strollers, and stroller wagons perfect for exploring Punta Cana\'s beautiful beaches and resorts with ease and comfort.',
      enabled: true,
    },
    {
      name: 'Car Seats',
    ...
    },
    {
      ...
      icon: '🏖️',
      description: 'Beach tents, sand toys, outdoor play equipment, and seasonal gear for family fun in the sun. Perfect for Punta Cana\'s pristine beaches.',
      enabled: true, // Punta Cana is a beach destination
    },
    {
    ...
    },
    {
      name: 'Pet Gear',
      icon: '🐾',
      description: 'Travel-friendly pet gear and accessories for your furry family members during your Punta Cana vacation.',
      enabled: true,
    },
  ],
  ...
  faqs: [
    ...
    {
      question: 'How much does it cost to rent a crib in Punta Cana?',
    ...
    {
      question: 'How do I get my baby to sleep well on vacation in Punta Cana?',
  ...
  // Pricing info - Based on content: Full-Size Crib From $50/day, Stroller Wagon From $35/day
  pricing_info: {
    crib_min: 50,
    crib_max: 50, // Specific pricing shown
    car_seat_min: null,
    car_seat_max: null,
    stroller_min: 35,
    stroller_max: null,
  ...
  // SEO
  seo_title: 'Baby Equipment Rentals in Punta Cana | TopTours.ai',
  seo_description: 'Rent baby equipment in Punta Cana: strollers, car seats, cribs, and more delivered to your hotel or vacation rental. Clean, safe, and insured gear from BabyQuip.',
  seo_keywords: [
    'baby equipment rental Punta Cana',
    'baby gear rental Punta Cana',
    'stroller rental Punta Cana',
    'car seat rental Punta Cana',
    'crib rental Punta Cana',
    'baby equipment Punta Cana',
    'baby gear rental Dominican Republic',
    'baby equipment rental Punta Cana Dominican Republic',
  ],
  
  is_active: true,
};

async function seedPuntaCana() {
  try {
    console.log('🌱 Seeding Punta Cana baby equipment rentals data...');
    
    // Upsert data into Supabase
    const { data, error } = await supabase
      .from('baby_equipment_rentals')
      .upsert({
        destination_id: puntaCanaData.destination_id.toLowerCase(),
        hero_title: puntaCanaData.hero_title || null,
        hero_description: puntaCanaData.hero_description || null,
        hero_tagline: puntaCanaData.hero_tagline || null,
        product_categories: puntaCanaData.product_categories ? JSON.parse(JSON.stringify(puntaCanaData.product_categories)) : null,
        intro_text: puntaCanaData.intro_text || null,
        rates_note: puntaCanaData.rates_note || null,
        faqs: puntaCanaData.faqs ? JSON.parse(JSON.stringify(puntaCanaData.faqs)) : null,
        pricing_info: puntaCanaData.pricing_info ? JSON.parse(JSON.stringify(puntaCanaData.pricing_info)) : null,
        seo_title: puntaCanaData.seo_title || null,
        seo_description: puntaCanaData.seo_description || null,
        seo_keywords: puntaCanaData.seo_keywords || null,
        is_active: puntaCanaData.is_active !== undefined ? puntaCanaData.is_active : true,
      }, {
    ...
    if (error) {
      console.error('❌ Error seeding Punta Cana data:', error);
      console.error('Error details:', JSON.stringify(error, null, 2));
      process.exit(1);
    }
    
    console.log('✅ Successfully seeded Punta Cana baby equipment rentals!');
