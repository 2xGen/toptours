...
import V3ToursListingClient from '@/components/destination/V3ToursListingClient';
...
export default function V3DestinationHubClient({
  destination,
  destinationSlug,
  categories = [],
  tours = [],
}) {
  const hubUrl = `/destinations/${destinationSlug}`;
  const toursUrl = `/destinations/${destinationSlug}/tours`;
  ...
  return (
    <>
        ...
        <div className="container relative z-10 mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid lg:grid-cols-[1fr,320px] gap-10 lg:gap-12 items-center">
            <div>
              ...
              {heroSubtitle && (
                <p className="mt-4 text-lg text-white/90 max-w-2xl">{heroSubtitle}</p>
              )}
              <div className="mt-8 flex flex-wrap gap-3">
                ...
                <Link
                  href={toursUrl}
                  className="inline-flex items-center justify-center gap-2 px-6 py-3 rounded-xl font-semibold text-white bg-white/15 border border-white/30 hover:bg-white/25 transition-colors"
                >
                  All tours
                  <ArrowRight className="h-5 w-5" />
                </Link>
            ...
            {categories.length > 0 && (
              <div className="bg-white/95 backdrop-blur-sm rounded-2xl shadow-xl border border-white/20 p-5 lg:p-6">
                <h3 className="font-poppins font-bold text-gray-900 text-lg mb-3">Quick navigation</h3>
                <p className="text-sm text-gray-600 mb-4">Top categories</p>
        ...
        <div className="container mx-auto px-4 sm:px-6 lg:px-8 max-w-6xl">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
            ...
            <div className="min-w-[220px]">
              <label htmlFor="v3-destination-category-filter" className="block text-sm font-semibold text-gray-700 mb-2">
                Category
              </label>
              <select
                id="v3-destination-category-filter"
          ...
          <div className="mt-6">
            <V3ToursListingClient tours={toursToShow} destinationName={destination.name} />
          </div>

          {totalTours > TOURS_PER_PAGE && (
            <div className="mt-8 text-center">
              <Link
                href={toursUrl}
                className="inline-flex items-center gap-2 px-6 py-3 rounded-xl font-semibold text-white sunset-gradient hover:opacity-95 transition-opacity shadow-lg"
              >
      ...
      <section id="categories" className="py-16 lg:py-20 bg-white" aria-labelledby="categories-heading">
        <div className="container mx-auto px-4 sm:px-6 lg:px-8 max-w-6xl">
          <div className="text-center mb-12">
            ...
            <h2 id="categories-heading" className="mt-4 font-poppins font-bold text-2xl sm:text-3xl lg:text-4xl text-gray-900">
              Popular categories
            </h2>
            <p className="mt-3 text-gray-600 max-w-xl mx-auto">Find the right type of experience for you.</p>
          </div>
              ...
              <div className="space-y-3">
                {faqs.map((faq, i) => (
                  ...
                  >
                    ...
                    <div className="px-5 pb-5 pt-0 text-gray-600 leading-relaxed">{faq.answer}</div>
                  </details>
                ))}
              </div>
            </div>
