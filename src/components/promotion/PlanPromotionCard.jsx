...
export default function PlanPromotionCard({ planId, initialScore = null, compact = false, planData = null }) {
  ...
  const { account, loading: accountLoading, refreshAccount } = usePromotion(compact);
  ...
  const loadScore = async () => {
    try {
      ...
      const response = await fetch(`/api/internal/promotion/plan-score?planId=${planId}`);
      if (response.ok) {
        const data = await response.json();
        setScore(data);
      } else {
        // If no score exists, set to default
        setScore({
          planId,
          total_score: 0,
          monthly_score: 0,
      ...
      console.error('Error loading plan score:', error);
  ...
  const handleSpendPoints = async () => {
    if (!account) {
      toast({
        ...
        description: 'Please sign in to promote plans with your daily points.',
        variant: 'destructive',
      });
      setShowModal(false);
      return;
    ...
    try {
      ...
      toast({
        title: '🎉 Points added!',
        description: `You've boosted this plan with ${points} points!`,
      });

      // Prepare share data
      const planUrl = typeof window !== 'undefined' ? window.location.href : '';
      const planName = planData?.title || `this plan`;
  ...
  const handleALaCartePurchase = async (packageName) => {
    ...
    try {
      // Get current page URL or construct plan URL for redirect after payment
      const returnUrl = typeof window !== 'undefined' 
        ? window.location.href 
        : `/plans/${planData?.slug || planId}`;

      // Call API to create Stripe checkout session
      const response = await fetch('/api/internal/promotion/purchase-a-la-carte-plan', {
        method: 'POST',
  ...
  // Don't render if planId is missing
  if (!planId) {
    return null;
  }
            ...
            >
              ...
              <div className="mb-4">
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <h3 className="text-lg font-bold text-gray-900">Boost This Plan</h3>
                </div>
              ...
              {accountLoading ? (
                ...
                <div className="text-center py-4">
                  <p className="text-sm text-gray-600 mb-4">
                    Sign in to boost this plan with your daily points
                  </p>
                  <a
                    href="/auth"
  ...
  // Full version for sidebar (with animations) - EXACT same as TourPromotionCard
  return (
    <div className="space-y-4">
